...
| D/V、E/V | 资产负债表 + 市值 |
| β 来源 | Damodaran 行业 β re-lever（优先）；自算 β 作对照值 |
| ERP | Damodaran ctryprem.csv（按国家）|
| gt = min(Rf, GDP) | Rf 来自 macro.db；GDP 硬编码 CN=4.5%、US/HK=4.0% |

⚠️ 若 `interest_expense` / `pretax_income` 缺失，自动降级为 L2（CAPM），输出中说明降级原因。
...
脚本自动：
- 构建 base / bull / bear 三套场景
- 以 L3 WACC 作为折现率 `r`，同时存储 `wacc_l2_sanity`
- 写入 `micro.db` 的 `scenarios` 表
